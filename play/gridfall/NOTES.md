...
- Converted boot to `GGKit.hiDpi.phaser` with `Phaser.Scale.NONE`, retained render defaults and existing dense texture baking, and installed dense Phaser text creation with inverse object scale.
- Layout and board geometry now use Phaser scale dimensions normalized by `cfg.ggDpr`; the camera zooms and centers on the board viewport midpoint.
- Gameplay screenshot, render-loop probe, and tile-swap input proof could not be completed because the local browser infrastructure was unavailable.
