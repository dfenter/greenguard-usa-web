...
(function () {
  ...
  function makeTexture(sceneRef, name, w, h, draw) {
    ...
    draw(g, w, h); g.generateTexture(name, Math.round(w * RETINA_FACTOR), Math.round(h * RETINA_FACTOR)); g.destroy(); return name;
  }
  function addText(sceneRef, x, y, text, size, color, weight, origin) {
    return sceneRef.add.text(x, y, text, { fontFamily: FONT, fontSize: size + 'px', fontStyle: weight >= 800 ? '900' : '700', color: color, resolution: RETINA_FACTOR }).setOrigin(origin == null ? 0 : origin);
  }
  ...
  var ForgelockScene = class extends Phaser.Scene {
    ...
    makeUi() {
      this.ui.chrome = this.add.image(0, 0, makeChrome(this, SETS[0])).setOrigin(0).setScale(1 / RETINA_FACTOR).setDepth(-10);
    ...
    createEffects() {
      var self = this; ['match', 'cascade', 'reward'].forEach(function (kind) { for (var i = 0; i < 20; i++) { var sprite = self.add.image(0, 0, makeEffectTexture(self, kind)).setScale(1 / RETINA_FACTOR).setVisible(false).setDepth(kind === 'reward' ? 12 : 8); self.effectPools[kind].push({ sprite: sprite, active: false, x: 0, y: 0, vx: 0, vy: 0, life: 0, max: 0, scale: 1, spin: 0 }); } });
    }
    applyInitialBoard() {
    ...
    startBoard(index) {
      ...
      this.layout = { pw: GRID * CELL + 36, ph: GRID * CELL + 36, pad: 18, bx: (W - (GRID * CELL + 36)) / 2, by: 132 }; this.ui.board = this.add.image(this.layout.bx, this.layout.by, makeBoardTexture(this, this.board, this.set, this.layout)).setOrigin(0).setScale(1 / RETINA_FACTOR).setDepth(0); this.world.push(this.ui.board); this.makeBoardObjects();
    ...
    makeBoardObjects() {
      var self = this;
      this.board.grid.forEach(function (row, y) { row.forEach(function (tile, x) { var sprite = self.add.image(0, 0, makeTileTexture(self, tile.color, tile.state === 'locked')).setScale(1 / RETINA_FACTOR).setDepth(5); self.world.push(sprite); self.renderRecords.tiles.push({ sprite: sprite, x: x, y: y }); var selected = self.add.rectangle(0, 0, CELL - 6, CELL - 6, self.set.accent, .16).setOrigin(.5).setStrokeStyle(2, self.set.accent, .9).setDepth(6).setVisible(false); self.world.push(selected); self.renderRecords.selection.push(selected); var preview = self.add.rectangle(0, 0, CELL - 9, CELL - 9, C.white, 0).setOrigin(.5).setStrokeStyle(2, self.set.second, .6).setDepth(6).setVisible(false); self.world.push(preview); self.renderRecords.preview.push(preview); }); });
      this.board.grid.forEach(function (row, y) { row.forEach(function (_tile, x) { var forge = self.add.image(0, 0, makeForgeTexture(self, (self.boardIndex + x + y) % COLORS.length)).setScale(1 / RETINA_FACTOR).setDepth(3).setAlpha(.28); self.world.push(forge); self.renderRecords.forges.push({ sprite: forge, cell: { x: x, y: y }, color: (self.boardIndex + x + y) % COLORS.length }); }); });
      var addMechanic = function (kind, data) { var sprite = self.add.image(0, 0, makeMechanicTexture(self, kind, data)).setScale(1 / RETINA_FACTOR).setDepth(4); self.world.push(sprite); self.renderRecords.mechanics.push({ sprite: sprite, kind: kind, data: data }); };
  ...
  var config = { type: Phaser.AUTO, parent: document.body, width: W, height: H, backgroundColor: '#0a1422', scale: { mode: Phaser.Scale.FIT, autoCenter: Phaser.Scale.CENTER_BOTH }, render: { antialias: true, roundPixels: false, powerPreference: 'high-performance', batchSize: 2048 }, fps: { target: 60, min: 30 }, scene: [ForgelockScene] };
