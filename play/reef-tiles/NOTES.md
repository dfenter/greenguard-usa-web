...
- Converted the parented game to `GGKit.hiDpi.phaser`, `Phaser.Scale.NONE`, and `cfg.ggDpr`; logical layout now derives from the scale dimensions, with the camera centered on the design midpoint. Existing device-scale canvas bakes and explicit display sizes were retained.
