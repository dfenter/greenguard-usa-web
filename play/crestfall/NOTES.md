...
- **Measured, not expected.** Release gate at deviceScaleFactor 3, run serially at concurrency 1 from `ue-port-studio/aaa/harness` against a private local server: `node release_gate.mjs http://localhost:8791 1 crestfall`.
- Gate verdict: **READY**. Measured `canvas.width / getBoundingClientRect().width` = **3.44x** (gate floor 2.85), sampled late by the gate, well after the point where a RESIZE parent poll would have reverted it. Backing store 1536x1344 in a 446x390 CSS box.
- Real gameplay frame: **29831 distinct colours** (8-bit), flattest colour 28.5% of the frame. The frame was compared side by side against the same interaction at deviceScaleFactor 1: layout, spacing and art are pixel-proportional, only the sampling is denser.
- Recipe: none of the three Phaser scale recipes applies literally. Crestfall is a hand-drawn Canvas2D title; Phaser owns only the loop and the canvas, and every pixel is painted by `Game._draw()` in the 768x672 (256x224 NES at SCALE 3) design space. So the canvas backing store and the Phaser game size are both multiplied by an INTEGER retina factor and `_draw()` opens with `ctx.scale(dpr, dpr)`. The CSS box is left entirely to the stylesheet's `min(100vw, 114.2857dvh)` rule, which is what actually sizes the canvas on screen.
- Factor: `Math.min(3, Math.ceil(GGKit.hiDpi.factor(W, H)))`. `hiDpi.factor` returns 1.74 on an 844x390 dpr-3 panel (the design box is wider than the letterboxed display box). Rounded UP to a clean 2 on purpose: this is pixel art with `image-rendering: pixelated`, and a fractional factor puts visible seams between the 3x-scaled NES pixels. The result overshoots the panel's own pixel count by ~15% in exchange for perfect integer scaling, and stays well inside the dpr-3 ceiling.
- `type` stays `Phaser.CANVAS` and cannot change: the title draws through a 2D context it fetches from the shared canvas. Fill cost is therefore 4x what it was. Measured on this GPU-less Mac the loop still advances normally at DPR 3 and the gate's ALIVE check passes; on real hardware this is a non-issue at 1536x1344.
- No `resolution` key anywhere (Phaser 3 removed it after 3.16 and silently ignores it), and no `source.resolution` on any texture.
