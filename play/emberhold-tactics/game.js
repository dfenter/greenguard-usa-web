...
(function () {
  ...
  skirmishFromUrl = Math.max(1, Math.min(5, Math.floor(skirmishFromUrl)));
  var liveScene = null;
  ...
  var profileDefault = {version:1, unlocked:1, medals:{}, tutorialDone:false, bestRounds:{}};
  function validMapKey(key) { return /^[1-5]$/.test(key); }
  ...
  function validProfile(o) {
    ...
    if (!Number.isInteger(o.version) || o.version !== 1) return false;
    if (!Number.isInteger(o.unlocked) || o.unlocked < 1 || o.unlocked > 5) return false;
    if (!validScoreMap(o.medals, 0, 3) || !validScoreMap(o.bestRounds, 1, 9999)) return false;
    return typeof o.tutorialDone === 'boolean' && Object.keys(o).every(function (key) { return ['version','unlocked','medals','tutorialDone','bestRounds'].indexOf(key) >= 0; });
  }
  var profile;
  ...
  profile = kit.save.get(profileDefault);
  if (!validProfile(profile)) profile = JSON.parse(JSON.stringify(profileDefault));
  ...
  function mapIdForValue(v) { var n = Number(v); if (Number.isFinite(n)) return clamp(Math.floor(n), 1, 5); var hit = Object.keys(Sim.MAPS).find(function (k) { return Sim.MAPS[k].id === v; }); return hit ? Number(hit) : 1; }

  var ICONS = {Attack:'⚔', 'Shield Bash':'⬟', Rampart:'⬢', 'Aimed Shot':'➶', Pin:'⌁', Fireball:'✹', 'Frost Lance':'❄', Heal:'✚', Bless:'✦', Chakra:'◎', 'Wave Fist':'◉', Backstab:'✧', 'Steal Tempo':'↯', 'Ember Sigil':'◇', 'Ruin Wave':'✺'};
  ...
  function EmberholdScene() {
    ...
    this.pinch = null; this.lastTap = {time:0,x:0,y:0}; this.confirmKey = null; this.uiKey = ''; this.coachTimer = 0; this.views = new Map(); this.fxCursor = 0; this.shakeX = 0; this.shakeY = 0; this.damageFlash = 0; this.dangerMusic = false; this.cursorOwner = ''; this.gamepadLatch = {};
  ...
  EmberholdScene.prototype.preload = function () {
    this.load.audio('ambient', 'assets/ambient_hum.mp3');
    this.load.audio('intensity', 'assets/ambient_intensity.mp3');
    this.load.audio('select', 'assets/sfx_select.mp3');
    this.load.audio('move', 'assets/sfx_move.mp3');
    this.load.audio('clash', 'assets/sfx_clash.mp3');
    this.load.audio('damage', 'assets/sfx_damage.mp3');
    this.load.audio('confirm', 'assets/sfx_confirm.mp3');
    this.load.audio('victory', 'assets/sfx_victory.mp3');
    this.load.audio('defeat', 'assets/sfx_defeat.mp3');
    this.load.audio('pickup', 'assets/sfx_pickup.mp3');
  ...
  EmberholdScene.prototype.startSkirmish = function (skirmish, seed) {
    ...
    this.accumulator = 0; this.visualTime = 0; this.confirmKey = null; this.panX = 0; this.panY = 0; this.cursor = null; this.pinch = null; this.fxCursor = 0; this.shakeX = 0; this.shakeY = 0; this.dangerMusic = false;
    ...
    kit.input.clearAll();
    this.sim = Sim.create(seed, skirmish); this.sim.start(); window.__et.state = this.sim.state;
  ...
  EmberholdScene.prototype.clearDeadViews = function () {
    ...
    this.views.forEach(function (v, id) { if (!alive.has(id)) { v.sprite.setVisible(false); v.label.setVisible(false); v.name.setVisible(false); } }, this);
  };
  ...
  EmberholdScene.prototype.consumeEvents = function () {
    ...
    for (var i = 0; i < events.length; i++) {
      ...
      if (e.type === 'move') { this.setUnitState(e.unitId, 'move', 260); this.spawnBurst(e.path[e.path.length - 1].c, e.path[e.path.length - 1].r, '#55c6ff', 5, 'trail'); this.emitSound('move', .35); continue; }
      if (e.type === 'attack') {
        this.setUnitState(e.unitId, 'attack', 300); this.emitSound('clash', .62); out = e.outcomes || [];
        for (var j = 0; j < out.length; j++) { target = this.sim.state.units.find(function (u) { return u.id === out[j].targetId; }); if (!target) continue; p = this.tilePoint(target.c, target.r); this.spawnBurst(target.c, target.r, out[j].hit ? (out[j].crit ? '#ffd36c' : '#ff6683') : '#94a8b0', out[j].hit ? 14 : 5, out[j].hit ? 'impact' : 'miss'); this.spawnFloat(target.c, target.r, out[j].hit ? (out[j].crit ? 'CRIT ' : '') + out[j].damage : 'MISS', out[j].hit ? (out[j].crit ? '#ffd36c' : '#f6f8e6') : '#aab8bc', out[j].hit ? 20 : 15); if (out[j].hit) { kit.juice.shake(out[j].crit ? 10 : 5, out[j].crit ? 240 : 160); kit.juice.hitStop(out[j].crit ? 85 : 48); } }
      ...
      if (e.type === 'terrain') { this.spawnBurst(e.c, e.r, '#c9a5ff', 16, 'rune'); this.spawnFloat(e.c, e.r, 'SIGIL', '#d7c0ff', 14); continue; }
      ...
      if (e.type === 'reinforcements') { this.showToast('REINFORCEMENTS DROPPED'); this.spawnBurst(8, 5, '#ff6683', 20, 'drop'); continue; }
  ...
  EmberholdScene.prototype.finishBattle = function (winner, rounds) {
    var playerLeft = this.sim.state.units.filter(function (u) { return u.team === 'player' && u.alive; }).length;
    var lost = 4 - playerLeft, medal = winner === 'player' ? (rounds <= 7 && lost === 0 ? 3 : rounds <= 11 && lost <= 1 ? 2 : 1) : 0;
    if (winner === 'player') { profile.medals[this.sim.state.skirmish] = Math.max(profile.medals[this.sim.state.skirmish] || 0, medal); profile.bestRounds[this.sim.state.skirmish] = Math.min(profile.bestRounds[this.sim.state.skirmish] || 999, rounds); if (this.sim.state.skirmish < 5) profile.unlocked = Math.max(profile.unlocked, this.sim.state.skirmish + 1); profile.tutorialDone = true; kit.save.set(profile); }
    this.clearTransients(); $('coach').classList.add('faded');
    $('banner-title').textContent = winner === 'player' ? (this.sim.state.skirmish === 5 ? 'FINALE CLEAR' : 'SKIRMISH CLEAR') : 'LINE FALLEN';
    var best = profile.bestRounds[this.sim.state.skirmish] ? ' · BEST ' + profile.bestRounds[this.sim.state.skirmish] : '';
    $('banner-text').textContent = winner === 'player' ? 'Round ' + rounds + ' · ' + medal + ' MEDAL · ' + lost + ' UNITS LOST' + best : 'The Emberhold line needs another push.';
    $('next-map').disabled = winner !== 'player' || this.sim.state.skirmish >= 5; $('next-map').textContent = this.sim.state.skirmish >= 5 ? 'CAMPAIGN COMPLETE' : 'NEXT SKIRMISH';
    $('banner').hidden = false; this.emitSound(winner === 'player' ? 'victory' : 'defeat', winner === 'player' ? .85 : .6); this.syncUI(true);
  };
  ...
  EmberholdScene.prototype.drawTile = function (c, r) {
    var tile = this.sim.state.board[r * N + c], p = this.tilePoint(c, r), base = tile.terrain === 'water' ? '#1f6d8c' : tile.terrain === 'sand' ? '#a98b51' : tile.terrain === 'stone' ? '#667984' : '#2f745a';
    var hi = tile.terrain === 'water' ? '#4aa3bb' : tile.terrain === 'sand' ? '#caa966' : tile.terrain === 'stone' ? '#8298a0' : '#4e9c75';
    ...
    if (tile.h > 0) { var side = {x:p.x, y:p.y + TILE_H * this.zoom / 2}; this.boardG.fillStyle(color(base), .5); this.boardG.fillTriangle(p.x - TILE_W * this.zoom / 2, p.y, p.x, p.y + TILE_H * this.zoom / 2, p.x, side.y + tile.h * EXTRUDE * this.zoom); this.boardG.fillStyle(color('#102a34'), .42); this.boardG.fillTriangle(p.x, p.y + TILE_H * this.zoom / 2, p.x + TILE_W * this.zoom / 2, p.y, p.x + TILE_W * this.zoom / 2, p.y + tile.h * EXTRUDE * this.zoom); }
    this.boardG.fillStyle(color(hi), .08); this.boardG.fillCircle(p.x - 9 * this.zoom, p.y - 5 * this.zoom, 5 * this.zoom);
  };
  EmberholdScene.prototype.drawHighlights = function () {
    ...
    if (s.mode === 'move') this.sim.reachable(u).forEach(function (entry) { overlay(entry.c, entry.r, alphaHex('#238cff', .3), alphaHex('#70d8ff', .72)); });
  ...
  EmberholdScene.prototype.drawPickups = function () {
    ...
    this.sim.state.fields.forEach(function (f) { var q = self.tilePoint(f.c, f.r); self.fxG.lineStyle(2, color('#c9a5ff'), .7); self.fxG.strokeCircle(q.x, q.y, 17 * self.zoom); self.fxG.fillStyle(color('#b66cff'), .12); self.fxG.fillCircle(q.x, q.y, 14 * self.zoom); });
  };
  ...
  EmberholdScene.prototype.drawUnit = function (u) {
    ...
    if (!u.alive) { if (deadView) { deadView.sprite.setVisible(false); deadView.label.setVisible(false); deadView.name.setVisible(false); } return; }
    ...
    var spriteScale = u.boss ? 1.18 : 1, spriteBob = view.state === 'move' ? Math.sin(this.visualTime / 55) * 4 : motion;
    view.sprite.setTexture(spriteKey).setPosition(p.x + this.shakeX, p.y - 37 * scale + spriteBob + this.shakeY).setDisplaySize(76 * scale * spriteScale, 76 * scale * spriteScale).setAlpha(statePulse).setRotation(view.state === 'attack' ? Math.sin(this.visualTime / 45) * .12 : 0).setVisible(true);
  ...
  EmberholdScene.prototype.render = function () {
    ...
    this.boardG.fillStyle(color(this.sim.state.map.theme === 'night' ? '#081326' : '#0d1d2a'), 1); this.boardG.fillRect(0, 0, W, H);
    this.boardG.fillStyle(color(this.sim.state.map.theme === 'finale' ? '#321a27' : '#142b3b'), .32); this.boardG.fillCircle(W / 2, 365, 460);
    ...
    this.drawFx();
    this.fxG.lineStyle(1, color('#79e8ef'), .16); this.fxG.strokeRect(18, 86, W - 36, H - 176);
  ...
  EmberholdScene.prototype.syncUI = function (force) {
    ...
    setTextIfChanged($('mission-name'), map.name); setTextIfChanged($('mission-state'), s.ended ? (s.winner === 'player' ? 'FIELD SECURED' : 'LINE FALLEN') : (a ? (a.team === 'player' ? 'BLUE' : 'RED') : 'WAIT') + ' · R' + s.round);
  ...
  EmberholdScene.prototype.previewText = function (spec, c, r) { var s = this.sim.state, u = s.active, t = this.sim.unitAt(c, r); if (spec.kind === 'heal' && t) return 'HEAL +' + Math.min(t.maxHp - t.hp, Math.round(u.ma * spec.factor)) + ' HP'; if (spec.kind === 'buff' && t) return 'BLESS +3 PA +8 ACC'; if (spec.kind === 'terrain') return 'MARK BURNING GROUND'; if (spec.aoe) return '3 x 3 AREA'; if (!t) return 'TARGET TILE READY'; var p = this.sim.preview(u, spec, t); return p.hit + '% HIT · ' + p.damage + ' EXPECTED · ' + p.dir; };
  EmberholdScene.prototype.renderActionPanel = function () {
    ...
    var self = this, names = ['Attack'].concat((jobSpec(u).abilities || []).filter(function (name) { return name !== 'Attack'; }));
    names.forEach(function (name) { var spec = self.sim.abilitySpec(u, name), b = document.createElement('button'); b.className = 'action-btn ' + (name === 'Attack' ? 'attack' : spec.kind === 'heal' || spec.kind === 'buff' ? 'support' : spec.kind === 'terrain' ? 'terrain' : ''); b.disabled = u.mp < spec.cost; b.title = name + (spec.cost ? ' · ' + spec.cost + ' MP' : '') + (spec.desc ? ' · ' + spec.desc : ''); b.setAttribute('aria-label', b.title); b.innerHTML = '<b><span class="action-icon" aria-hidden="true">' + abilityIcon(name) + '</span><span class="action-label">' + name.toUpperCase() + '</span></b>'; b.addEventListener('pointerdown', function (e) { self.claimPointer(e, 'action'); }); b.addEventListener('click', function () { self.userGesture(); if (self.sim.beginAction(name)) { self.emitSound('confirm', .35); self.confirmKey = null; self.syncUI(true); self.render(); } }); panel.appendChild(b); });
    var end = document.createElement('button'); end.className = 'action-btn'; end.title = 'Wait and finish turn safely'; end.setAttribute('aria-label', end.title); end.innerHTML = '<b><span class="action-icon" aria-hidden="true">⌛</span><span class="action-label">WAIT</span></b>'; end.addEventListener('pointerdown', function (e) { self.claimPointer(e, 'action'); }); end.addEventListener('click', function () { self.userGesture(); self.sim.endTurn(); self.syncUI(true); self.render(); }); panel.appendChild(end);
    if (s.mode === 'action' && u.moveUsed) { var undo = document.createElement('button'); undo.className = 'action-btn'; undo.title = 'Undo move'; undo.setAttribute('aria-label', undo.title); undo.innerHTML = '<b><span class="action-icon" aria-hidden="true">↶</span><span class="action-label">UNDO</span></b>'; undo.addEventListener('pointerdown', function (e) { self.claimPointer(e, 'action'); }); undo.addEventListener('click', function () { self.sim.undoMove(); self.syncUI(true); self.render(); }); panel.appendChild(undo); }
    if (s.mode !== 'aim') { ['Surge','Rally'].forEach(function (name) { var b = document.createElement('button'); b.className = 'action-btn tempo'; b.disabled = s.tempo[u.team] < (name === 'Surge' ? 2 : 3); b.title = name + ' · costs ' + (name === 'Surge' ? 2 : 3) + ' tempo'; b.setAttribute('aria-label', b.title); b.innerHTML = '<b><span class="action-icon" aria-hidden="true">✦</span><span class="action-label">' + name.toUpperCase() + '</span></b>'; b.addEventListener('pointerdown', function (e) { self.claimPointer(e, 'action'); }); b.addEventListener('click', function () { self.userGesture(); self.sim.spendTempo(name); self.syncUI(true); self.render(); }); panel.appendChild(b); }); }
  };
  EmberholdScene.prototype.updateMenu = function () {
    var grid = $('scenario-grid'); if (!grid) return;
    if (grid.dataset.ready !== '1') { grid.dataset.ready = '1'; var self = this; Object.keys(Sim.MAPS).forEach(function (k) { var n = Number(k), b = document.createElement('button'); b.dataset.skirmish = String(n); b.addEventListener('pointerdown', function (e) { self.claimPointer(e, 'menu'); }); b.addEventListener('click', function () { self.userGesture(); self.closeMenu(); self.startSkirmish(n, urlSeed); }); grid.appendChild(b); }); }
    Array.from(grid.children).forEach(function (b) { var n = Number(b.dataset.skirmish), map = Sim.MAPS[n], best = profile.bestRounds[n] ? ' · BEST ' + profile.bestRounds[n] : ''; b.textContent = (profile.medals[n] ? '★'.repeat(profile.medals[n]) + ' ' : '') + 'S' + n + ' ' + map.name.replace('EMBERHOLD CORE','FINALE') + best; b.className = n > profile.unlocked ? 'locked' : n === (this.sim && this.sim.state.skirmish) ? 'selected' : ''; b.disabled = n > profile.unlocked; }, this);
    if ($('menu-seed')) setTextIfChanged($('menu-seed'), 'SEED ' + (this.sim ? this.sim.state.seed : urlSeed)); if ($('menu-status')) setTextIfChanged($('menu-status'), 'SKIRMISH ' + (this.sim ? this.sim.state.skirmish : 1) + ' · ' + (profile.unlocked >= (this.sim ? this.sim.state.skirmish : 1) ? 'UNLOCKED' : 'LOCKED'));
  };
  EmberholdScene.prototype.userGesture = function () { try { kit.audio.resume(); kit.audio.music(this.dangerMusic ? 'intensity' : 'ambient', 500); } catch (e) {} };
  ...
  EmberholdScene.prototype.wireInput = function () {
    ...
    canvas.addEventListener('pointerdown', function (e) {
      self.userGesture(); if (self.sim.state.ended) return;
      window.setTimeout(function () { var p = kit.input.pointers.get(e.pointerId); if (!p) return; p.board = true; p.lastX = e.clientX; p.lastY = e.clientY; p.moved = false; var two = boardPointers(); if (two.length >= 2) self.pinch = {distance:Math.hypot(two[1].x - two[0].x, two[1].y - two[0].y), cx:(two[0].x + two[1].x) / 2, cy:(two[0].y + two[1].y) / 2}; }, 0);
    ...
    canvas.addEventListener('pointercancel', function () { self.pinch = null; });
  ...
  EmberholdScene.prototype.wireDom = function () {
    var self = this;
    $('menu-btn').addEventListener('pointerdown', function (e) { self.claimPointer(e, 'menu'); }); $('menu-btn').addEventListener('click', function () { self.userGesture(); self.openMenu(); });
    $('close-menu').addEventListener('pointerdown', function (e) { self.claimPointer(e, 'menu'); }); $('close-menu').addEventListener('click', function () { self.closeMenu(); });
    $('menu-sheet').addEventListener('click', function (e) { if (e.target === $('menu-sheet')) self.closeMenu(); });
    $('end-turn').addEventListener('pointerdown', function (e) { self.claimPointer(e, 'hud'); }); $('end-turn').addEventListener('click', function () { self.userGesture(); if (self.sim.endTurn()) { profile.tutorialDone = true; kit.save.set(profile); self.syncUI(true); self.render(); } });
    $('confirm-chip').addEventListener('pointerdown', function (e) { self.claimPointer(e, 'hud'); }); $('confirm-chip').addEventListener('click', function () { self.userGesture(); var s = self.sim.state; if (s.aim && s.action) { self.sim.confirmAction(s.aim.c, s.aim.r); self.confirmKey = null; self.syncUI(true); self.render(); } });
    $('cancel-chip').addEventListener('pointerdown', function (e) { self.claimPointer(e, 'hud'); }); $('cancel-chip').addEventListener('click', function () { self.sim.cancelAction(); self.confirmKey = null; self.syncUI(true); self.render(); });
    ...
    $('same-map').addEventListener('click', function () { $('banner').hidden = true; self.requestRestart(); }); $('next-map').addEventListener('click', function () { if (self.sim.state.skirmish < 5 && self.sim.state.winner === 'player') { $('banner').hidden = true; self.startSkirmish(self.sim.state.skirmish + 1, urlSeed); } });
    window.addEventListener('keydown', function (e) { if (e.metaKey || e.ctrlKey || e.altKey || !kit.input.keyDown(e.code) || e.repeat) return; var k = e.key.toLowerCase(); if (k === 'm') { if ($('menu-sheet').hidden) self.openMenu(); else self.closeMenu(); e.preventDefault(); return; } if (k === 'r') { self.requestRestart(); e.preventDefault(); return; } if (!$('menu-sheet').hidden) { if (k === 'escape') self.closeMenu(); return; } if (k === 'escape') { self.sim.cancelAction(); self.confirmKey = null; self.syncUI(true); self.render(); e.preventDefault(); return; } if (k === 'e') { if (self.sim.endTurn()) { profile.tutorialDone = true; kit.save.set(profile); self.syncUI(true); self.render(); } e.preventDefault(); return; } if (k === ' ' || k === 'enter') { self.activateCursor(); e.preventDefault(); return; } var v = {arrowup:[0,-1],w:[0,-1],arrowdown:[0,1],s:[0,1],arrowleft:[-1,0],a:[-1,0],arrowright:[1,0],d:[1,0]}[k]; if (v && self.sim.state.active) { self.moveCursor(v[0], v[1]); e.preventDefault(); } });
  };
  EmberholdScene.prototype.update = function (time, delta) {
    if (this.paused || !this.sim) return;
    this.visualTime += Math.min(delta, 50); this.accumulator += Math.min(delta, 50); this.coachTimer = Math.max(0, this.coachTimer - Math.min(delta, 50));
