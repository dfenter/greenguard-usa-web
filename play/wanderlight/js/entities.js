/* entities.js — Wren, enemies, projectiles, pickups.
...
const Entities = (() => {
  ...
  function tryMove(e, dx, dy, game, inset) {
    ...
    const wrenEscape = e.kind === 'wren' && embedded;
    ...
    const attempt = (nx, ny) => {
      ...
      if (wrenEscape || clear || reduces) { e.x = nx; e.y = ny; moved = true; }
    };
    if (dx) {
      const nx = e.x + dx;
      attempt(nx, e.y);
  ...
  const WREN_INSET = { x:2, x2:2, y:5, y2:1 };
  ...
  // ============================ WREN ============================
  function makeWren(px, py) {
    return {
      kind:'wren', x:px, y:py, w:16, h:16, dir:'down',
      ...
      update(game) {
        ...
        // Major pickups briefly lock Wren in the classic overhead item pose.
        if (this.hoist > 0) {
          this.moving = false;
          this.hoist--;
        ...
        // knockback overrides control
          ...
          const k = this.knock;
          tryMove(this, k.dx, k.dy, game, WREN_INSET);
          ...
          if (this.moving) {
            ...
            tryMove(this, dx * this.speed, dy * this.speed, game, WREN_INSET);
          }
          // attack
      ...
      hurt(game, dmg, srcx, srcy) {
        ...
        if (this.health <= 0) game.onWrenDead();
      },
      draw(ctx, ox, oy) {
        ...
        if (this.hoist > 0) {
          ...
          // one pixel while the item stays above Wren's head.
        ...
        const set = Sprites.get('wren')[this.dir];
  ...
  function makeEnemy(type, variant, tx, ty) {
    if (type === 'coilwyrm') return makeCoilwyrm(tx, ty);
    if (type === 'emberback') return makeEmberback(tx, ty, { boss:false, pair:false });
    if (type === 'haloswarm') return makeHaloSwarm(tx, ty);
    if (type === 'sable') return makeSable(tx, ty);
    if (type === 'hungryboomerkin') return makeHungryBoomerkin(tx, ty);
    const base = {
    ...
    if (type === 'emberorb') { base.anchorX = base.x + 8; base.anchorY = base.y + 8; }
    base.update = function(game) { (cfg.update || stonepeeperUpdate)(this, game); commonPost(this, game); };
  ...
  function commonPost(e, game) {
    ...
    if (!e.hidden && overlap(hitbox(e), hitbox(game.wren))) {
      if (e.etype === 'emberorb') {
        game.wren.swordDisabled = Math.max(game.wren.swordDisabled || 0, 80);
      } else if (e.etype === 'griphand') {
        if (game.onGriphandGrab) game.onGriphandGrab(e);
      } else if (!e.invulnerable) {
        game.wren.hurt(game, e.touchDmg, e.x + 8, e.y + 8);
      }
    }
  }
  ...
  function spriteFor(e) {
    switch (e.etype) {
      case 'stonepeeper': return Sprites.get('stonepeeper')[e.variant === 'blue' ? 'blue' : 'red'];
      case 'mossbrute':  return e.variant === 'blue' ? Sprites.get('mossbrute_blue') : Sprites.get('mossbrute');
      case 'springclaw': return Sprites.get('springclaw')[e.variant === 'blue' ? 'blue' : 'orange'];
      case 'sandburrow':  return Sprites.get('sandburrow');
      case 'reedripper':    return Sprites.get('reedripper');
      case 'glimmerbat':   return Sprites.get('glimmerbat');
      case 'boneguard': return Sprites.get('boneguard');
      case 'gel':     return Sprites.get('gel')[e.variant === 'blue' ? 'blue' : 'green'];
      case 'cinderhorn':   return Sprites.get('cinderhorn');
      case 'boomerkin':     return Sprites.get('boomerkin') ? (e.variant==='blue' ? Sprites.get('boomerkin').blue : Sprites.get('boomerkin').brown) : Sprites.get('mossbrute');
      case 'ironwarden':return Sprites.get('ironwarden') || Sprites.get('boneguard');
      case 'nightwarden':    return Sprites.get('nightwarden') || Sprites.get('boneguard');
      case 'veilcaster':   return Sprites.get('veilcaster') || Sprites.get('stonepeeper').red;
      case 'mireleecher':   return Sprites.get('mireleecher') || Sprites.get('sandburrow');
      case 'rushcoil':       return Sprites.get('sandburrow');
      case 'slimelet':        return Sprites.get('gel').green;
      case 'emberorb':     return Sprites.get('glimmerbat');
      case 'griphand': return Sprites.get('boneguard');
      default:        return Sprites.get('stonepeeper').red;
    }
  }
  ...
  function stonepeeperUpdate(e, game) {
    wander(e, game, 0.02);
    if (--e.shootTimer <= 0) {
  ...
  function mossbruteUpdate(e, game) {
    wander(e, game, 0.02);
    if (--e.shootTimer <= 0) {
  ...
  function springclawUpdate(e, game) {
    if (e.knock) return;
    if (e.jump > 0) {
      ...
      e.jvx += (game.wren.x > e.x ? 0.6 : -0.6);
    }
  }
  function sandburrowUpdate(e, game) {
    if (e.knock) return;
    if (e.state === 'hidden') {
      e.hidden = true;
    ...
    e.hidden = false;
    // move toward player
    const dx = game.wren.x - e.x, dy = game.wren.y - e.y;
  ...
  function reedripperUpdate(e, game) {
  ...
  // Glimmerbat: erratic flight, bursts of motion with pauses
  function glimmerbatUpdate(e, game) {
    if (e.knock) return;
    if (--e.t <= 0) {
  ...
  // Boneguard: cardinal wander, tougher, no projectile
  function boneguardUpdate(e, game) { wander(e, game, 0.03); }
  ...
  // Boomerkin: wanders, throws boomerang, waits for return
  function boomerkinUpdate(e, game) {
    if (e.boomOut) return;
    wander(e, game, 0.025);
    if (--e.shootTimer <= 0) {
  ...
  // Nightwarden: chases player, blocks front hits
  function nightwardenUpdate(e, game) {
    if (e.knock) return;
    if (--e.moveTimer <= 0) {
  ...
  // Veilcaster: visible state is deliberately phase-gated, like the NES enemy.
  function veilcasterUpdate(e, game) {
    const phase = e.phase;
    if (phase === 'invisible') {
    ...
    if (phase === 'shimmer') {
      ...
      if (--e.phaseTimer <= 0) {
        ...
        for (let i = 0; i < n; i++) {
          const dx = game.wren.x - e.x, dy = game.wren.y - e.y, m = Math.hypot(dx, dy) || 1;
          const offset = i ? 8 : 0;
  ...
  // Rushcoil: wander until Wren enters its row/column, then commit to a fast charge.
  function rushcoilUpdate(e, game) {
    if (e.knock) return;
    const sameRow = Math.abs((game.wren.y + 8) - (e.y + 8)) < 7;
    const sameCol = Math.abs((game.wren.x + 8) - (e.x + 8)) < 7;
    if (!e.charging && (sameRow || sameCol)) {
      e.charging = true;
      if (sameRow) e.dir = game.wren.x < e.x ? 'left' : 'right';
      else e.dir = game.wren.y < e.y ? 'up' : 'down';
    }
    if (e.charging) {
      const [vx, vy] = DIRS[e.dir];
  ...
  // Mireleecher: slow wander, eats Wren's magical shield on contact
  function mireleecherUpdate(e, game) {
    wander(e, game, 0.02);
    if (overlap(hitbox(e), hitbox(game.wren))) {
      e.eatTimer = (e.eatTimer || 0) + 1;
      if (e.eatTimer >= 3 && game.wren.hasMagicShield) {
        game.wren.hasMagicShield = false;
  ...
  // Cinderhorn: cardinal wander toward player, fires sword beams
  function cinderhornUpdate(e, game) {
    if (e.knock) return;
    if (--e.moveTimer <= 0) {
      e.moveTimer = game.randInt(30, 60);
      const dx = game.wren.x - e.x, dy = game.wren.y - e.y;
  ...
  const ENEMY_CFG = {
    stonepeeper: { init:(v)=>({ hp:1, speed:0.7, shootTimer:90, value: v==='blue'?2:1, dropClass:'minor' }), update:stonepeeperUpdate },
    mossbrute:  { init:(v)=>({ hp:2, speed:0.65, shootTimer:80, value:2, dropClass:'mid' }), update:mossbruteUpdate },
    springclaw: { init:()=>({ hp:1, speed:0, rest:30, jump:0, jvx:0, jvy:0, value:1, dropClass:'minor' }), update:springclawUpdate },
    sandburrow:  { init:()=>({ hp:2, speed:0.8, state:'hidden', hidden:true, timer:40, value:2, dropClass:'minor' }), update:sandburrowUpdate,
               draw:(e,ctx,ox,oy)=>{ if(e.hidden) return; defaultDraw(e,ctx,ox,oy); } },
    reedripper:    { init:()=>({ hp:2, speed:0, state:'up', hidden:false, timer:90, value:2, touchDmg:1, dropClass:'mid' }), update:reedripperUpdate,
               draw:(e,ctx,ox,oy)=>{ if(e.hidden) return; defaultDraw(e,ctx,ox,oy); } },
    glimmerbat:   { init:()=>({ hp:1, speed:0, t:1, vx:0, vy:0, value:1, dropClass:'minor' }), update:glimmerbatUpdate },
    boneguard: { init:()=>({ hp:2, speed:0.7, value:2, touchDmg:1, dropClass:'mid' }), update:boneguardUpdate },
    gel:     { init:()=>({ hp:1, speed:0, t:1, vx:0, vy:0, value:1, w:10, h:10, dropClass:'minor' }), update:gelUpdate },
    cinderhorn:   { init:()=>({ hp:4, speed:0.9, shootTimer:60, moveTimer:30, value:5, touchDmg:2, dropClass:'elite' }), update:cinderhornUpdate },
    boomerkin:  { init:(v)=>({ hp: v==='blue'?4:2, speed:0.7, shootTimer:80, boomOut:false, value:3, touchDmg:1, dropClass:'mid' }), update:boomerkinUpdate },
    ironwarden: { init:()=>({ hp:4, speed:0.4, shootTimer:9999, moveTimer:40, value:5, touchDmg:2, dropClass:'elite' }), update:ironKnuckleUpdate },
    nightwarden: {
      init:()=>({ hp:3, speed:0.6, moveTimer:40, value:4, touchDmg:2, dropClass:'elite' }),
      update:nightwardenUpdate,
    ...
    veilcaster:{ init:()=>({ hp:2, speed:0, phase:'invisible', phaseTimer:90, hidden:true, invulnerable:true, value:3, touchDmg:2, dropClass:'elite' }), update:veilcasterUpdate },
    mireleecher:{ init:()=>({ hp:2, speed:0.3, value:2, touchDmg:1 }), update:mireleecherUpdate },
    rushcoil:    { init:()=>({ hp:1, speed:0.8, value:1, touchDmg:1, charging:false, dropClass:'minor' }), update:rushcoilUpdate },
    slimelet:     { init:()=>({ hp:1, speed:0.5, value:2, touchDmg:1, dropClass:'mid' }), update:gelUpdate,
               hurt:(e,game,dmg,kx,ky)=>{
                 if (e.flash > 4 || e.invulnerable) return;
                 e.hp -= dmg; e.flash = 10; Sound.SFX.enemyHit();
                 if (e.hp > 0) return;
    ...
    emberorb:  { init:()=>({ hp:1, speed:0, value:0, touchDmg:0, hidden:false, invulnerable:true,
                           countsForClear:false, noDrops:true, orbit:0, anchorX:0, anchorY:0 }), update:emberorbUpdate,
               draw:(e,ctx,ox,oy)=>{
    ...
    griphand:{ init:()=>({ hp:2, speed:1.8, value:0, touchDmg:0, hidden:true, state:'dormant', phaseTimer:100,
                             countsForClear:false, noDrops:true }), update:griphandUpdate,
                 hurt:(e,game,dmg,kx,ky)=>enemyHurt(e,game,dmg,kx,ky) },
  };

  // Classic corner blade trap. It is a hazard, not an enemy: it never blocks
  // room clear and cannot be damaged. The only randomness is the Wren-facing
  ...
  function makeBladeTrap(corner) {
    ...
    return {
      ...
      update(game) {
        const l = game.wren, row = Math.abs((l.y + 8) - (this.y + 8)) < 6;
  ...
  // ============================ BOSS: Tidehorn ============================
  function makeTidehorn(tx, ty, opts = {}) {
    const e = makeEnemy('stonepeeper', 'red', tx, ty);  // reuse scaffolding
    const variant = opts.variant || 'green';
    const hp = variant === 'blue' ? 10 : variant === 'red' ? 8 : 6;
    e.etype = 'tidehorn'; e.boss = true; e.variant = variant;
    ...
    e.update = function(game) {
      ...
      if (overlap(hitbox(this), hitbox(game.wren)))
        game.wren.hurt(game, this.touchDmg, this.x+12, this.y+16);
    };
    e.draw = function(ctx, ox, oy) {
      const sp = Sprites.get('tidehorn')[this.variant] || Sprites.get('tidehorn').green;
  ...
  function makeHaloSwarm(tx, ty) {
    const core = {
      kind:'enemy', etype:'haloswarm', boss:false, miniboss:true,
      ...
      update(game) {
        ...
        if (overlap(hitbox(this), hitbox(game.wren))) game.wren.hurt(game, this.touchDmg, this.x+9, this.y+9);
      },
      hurt(game, dmg, kx, ky) {
    ...
    for (let i=0; i<n; i++) {
      ...
      const sat = {
        kind:'enemy', etype:'haloswarmSatellite', parent:core, x:core.x, y:core.y,
        ...
        update(game) {
          ...
          if (overlap(hitbox(this), hitbox(game.wren))) game.wren.hurt(game, this.touchDmg, this.x+6, this.y+6);
        },
        hurt(game, dmg, kx, ky) {
  ...
  // ================================ SABLE =================================
  // Sable is never timer-revealed. Each sword/beam hit is a deliberate stun;
  // only the fourth stun exposes him permanently, and only silver can finish.
  function makeSable(tx, ty) {
    const e = {
      kind:'enemy', etype:'sable', boss:true, variant:'sable',
      ...
      update(game) {
        ...
        if (this.stun > 0) {
          ...
          if (--this.moveTimer <= 0) {
            ...
            const dx=game.wren.x-this.x, dy=game.wren.y-this.y;
        ...
        if (this.stun <= 0 && --this.shootTimer <= 0) {
          ...
          const dx=game.wren.x-this.x, dy=game.wren.y-this.y, m=Math.hypot(dx,dy)||1;
        ...
        if (!this.hidden && overlap(hitbox(this),hitbox(game.wren))) game.wren.hurt(game,this.touchDmg,this.x+12,this.y+14);
      },
  ...
  // ============================ BOSS: Manymaw ============================
  function makeManymaw(tx, ty, opts = {}) {
    ...
    const e = {
      kind:'enemy', etype:'manymaw', boss:true, variant:'green',
      ...
      update(game) {
        ...
        for (const h of this.heads) {
          if (h.detached) {
            ...
            if (--h.shootTimer <= 0) {
              ...
              const dx = game.wren.x - h.x, dy = game.wren.y - h.y;
              const m = Math.hypot(dx, dy) || 1;
            ...
            if (overlap(hbox, hitbox(game.wren)))
              game.wren.hurt(game, 2, h.x, h.y);
          } else if (h.hp > 0) {
        ...
        if (overlap(hitbox(this), hitbox(game.wren)))
          game.wren.hurt(game, this.touchDmg, this.x + 24, this.y + 32);
  ...
  function makeEmberback(tx, ty, opts = {}) {
    const pair = !!opts.pair;   // harder variant: needs 3 bombs
    const e = {
      kind:'enemy', etype:'emberback', boss: opts.boss !== false, variant: pair ? 'red' : 'green',
      ...
      update(game) {
        ...
        if (overlap(hitbox(this), hitbox(game.wren)))
          game.wren.hurt(game, this.touchDmg, this.x + 13, this.y + 9);
      },
      ...
      update(game) {
        ...
        for (const h of aliveHeads) {
          ...
          if (--h.shootTimer <= 0) {
            ...
            const dx = game.wren.x - hx, dy = game.wren.y - hy;
            const m = Math.hypot(dx, dy) || 1;
          ...
          if (overlap(hb, hitbox(game.wren))) game.wren.hurt(game, 2, hb.x + 8, hb.y + 8);
        }
  ...
  // ============================ DUNGEON MINIBOSSES ============================
  function makeCoilwyrm(tx, ty) {
    const e = {
      kind:'enemy', etype:'coilwyrm', x:tilePx(tx), y:tilePx(ty), w:16, h:16,
      ...
      update(game) {
        ...
        for (let i = 0; i < this.segments.length; i++) {
          const s = this.segments[i]; s.x = this.x - i * 13; s.y = this.y + Math.sin((game.counters.coilwyrmT || 0) * 0.08 + i) * 7;
        }
        game.counters.coilwyrmT = (game.counters.coilwyrmT || 0) + 1;
        for (const s of this.segments) if (s.hp > 0 && overlap({x:s.x,y:s.y,w:14,h:14}, hitbox(game.wren))) game.wren.hurt(game, this.touchDmg, s.x+7, s.y+7);
      },
      hurt(game, dmg, kx, ky) {
        if (this.flash > 4) return;
  ...
  // ============================ BOSS: Mireeye ============================
  function makeMireeye(tx, ty) {
    const e = { kind:'enemy', etype:'mireeye', boss:true, x:tilePx(tx), y:tilePx(ty), w:32, h:24,
      ...
      update(game) {
        ...
        if (overlap(hitbox(this), hitbox(game.wren))) game.wren.hurt(game, 2, this.x+16, this.y+12);
      },
      hurt(game, dmg, kx, ky, src) {
  ...
  function makeBurrowerSmall(parent, x, y) {
    const e = { kind:'enemy', etype:'burrowerSmall', boss:false, x, y, w:14, h:14, hp:2, alive:true,
      speed:1.1, dir:1, touchDmg:1, noDrops:true,
      update(game) { this.x += this.dir*this.speed; if (this.x<16||this.x>226) this.dir*=-1; if (overlap(hitbox(this),hitbox(game.wren))) game.wren.hurt(game,1,this.x+7,this.y+7); },
  ...
  // ============================ BOSS: Burrower ============================
  function makeBurrower(tx, ty) {
    const e = { kind:'enemy', etype:'burrower', boss:true, x:tilePx(tx), y:tilePx(ty), w:40, h:40,
      ...
      shrink(game) {
        ...
        for (let i=0;i<this.childrenLeft;i++) game.spawn(makeBurrowerSmall(this, this.x+8+i*12, this.y+8));
        Sound.SFX.secret(); return true;
      },
      update(game) {
        if (this.flash>0) this.flash--;
        if (!this.large) { if (--this.shrinkTimer<=0) { this.large=true; for (const c of game.entities) if (c.etype==='burrowerSmall') c.alive=false; } return; }
        if (overlap(hitbox(this),hitbox(game.wren))) game.wren.hurt(game,2,this.x+20,this.y+20);
      },
  ...
  function makeHungryBoomerkin(tx, ty) {
    return { kind:'enemy', etype:'hungryboomerkin', x:tilePx(tx), y:tilePx(ty), w:16, h:24, hp:1, alive:true,
      countsForClear:false, invulnerable:true, gate:true,
      update(game) { if (overlap(hitbox(this), hitbox(game.wren))) game.onHungryBoomerkin(this); },
      ...
      update(game) {
        ...
        if (this.fromEnemy) {
          if (overlap(hitbox(this), hitbox(game.wren))) {
            ...
            if (l.hasShield && l.attackTimer <= 0 && (BLOCKABLE[this.ptype] || magical)) {
              const fromDir = Math.abs(this.vx) > Math.abs(this.vy)
                ? (this.vx > 0 ? 'left' : 'right')   // travelling right → hits Wren's left-facing shield
                : (this.vy > 0 ? 'up' : 'down');
              if (l.dir === fromDir) {
                this.alive = false;
          ...
          for (const en of game.entities) {
            if (en.kind === 'enemy' && en.alive !== false && (!en.hidden || en.etype === 'sable')) {
    ...
    return {
      ...
      update(game) {
        ...
        for (const en of game.entities) {
          ...
          if (en.kind === 'item' && en.alive !== false && this.owner.kind === 'wren' &&
              overlap(hitbox(this), hitbox(en))) {
            en.x = this.owner.x; en.y = this.owner.y;  // pull toward wren (never a Boomerkin)
          }
        }
      },
      draw(ctx, ox, oy) {
  ...
  function drawItem(it, ctx, ox, oy) {
    ...
    switch (it.item) {
      ...
      case 'shard': {
        ...
        // Lumen Shard: a faceted diamond
        for(let i=0;i<6;i++) ctx.fillRect(x+7-i,y+1+i,i*2+1,1);
        for(let i=0;i<7;i++) ctx.fillRect(x+2+i,y+7+i,(5-i)*2+3,1);
        ctx.fillStyle = f ? '#fffbd0':'#fff';
        ctx.fillRect(x+6,y+4,2,4); break; }
      case 'heartcontainer':
        ctx.fillStyle='#d82828';
  ...
  return {
    makeWren, makeEnemy, makeTidehorn, makeManymaw, makeEmberback, makeThorncrown, makeCoilwyrm, makeMireeye, makeBurrower, makeHaloSwarm, makeSable,
    makeProjectile, makeBoomerang, makeBomb, makeItem, makeFx, makeBladeTrap,
    overlap, hitbox, tryMove, WREN_INSET, ENEMY_INSET, DIRS,
  };
})();
