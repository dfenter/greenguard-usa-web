...
function validSave(value) {
  if (!value || value.v !== 1 || typeof value.tutorialDone !== 'boolean') return false;
...
function defaultSave() {
  return { v: 1, tutorialDone: false, medals: 0, machine: MACHINES[0].id, livery: 0, best: {}, cups: {} };
}
...
class CourseLayer {
  constructor(racer, data) {
    ...
    for (const feature of Array.isArray(data.features) ? data.features : []) {
      ...
      const pad = new THREE.Mesh(new THREE.BoxGeometry(track.width * .55, .06, 3.2), featureMaterials[feature.type]);
...
class IonwakeApp {
  constructor(trackData) {
    ...
    this.save = kit.save.get(defaultSave());
    ...
    this.pointerClaims = new Map(); this.previousKeys = new Set(); this.toastText = ''; this.toastTimer = 0; this.landingText = ''; this.tutorialTimer = 0; this.worldPacket = { carState: {}, rivals: [] };
    this.sim = { progress: .04, distance: .04, lap: 1, lapTime: 0, totalTime: 0, speed: 0, steering: 0, energy: 100, acceleration: 0, lateral: 0, boost: 0, airborne: 0, jumpQuality: '', contactTimer: 0, scrapeTimer: 0, previousProgress: .04, finished: false };
  ...
  startRace(mode = this.raceMode, trackIndex = this.trackIndex) {
    ...
    this.racer.world.mainCar.root.visible = false;
    for (const rival of this.racer.world.rivals) rival.root.visible = false;
    ...
    this.player = new HoverCraft({ name: machine.name, paint: livery.paint, accent: livery.accent }); this.racer.world.scene.add(this.player.root);
    if (mode === 'time-attack' && this.ghost) {
      this.ghostCraft = new HoverCraft({ name: 'time attack ghost', paint: 0x86f5ff, accent: 0xd8ffff, scale: .88 });
    ...
    this.rivals = [];
    for (let i = 0; i < 7; i += 1) { const rival = new HoverCraft({ name: 'AI ' + (i + 1), paint: rivalPalette[i], accent: 0xeaf7ff, scale: .92 }); this.racer.world.scene.add(rival.root); this.rivals.push(rival); }
    this.sim = { progress: .04, distance: .04, lap: 1, lapTime: 0, totalTime: 0, speed: 0, steering: 0, energy: machine.shield, acceleration: 0, lateral: 0, boost: 0, airborne: 0, jumpQuality: '', contactTimer: 0, scrapeTimer: 0, previousProgress: .04, finished: false };
    this.ai = this.rivals.map((_, i) => ({ progress: wrap01(.04 - (i + 1) * .013), distance: .04 - (i + 1) * .013, speed: 0, lateral: lineLateral(data, .04 - (i + 1) * .013), aggression: .35 + i * .075, skill: .92 + (i % 4) * .025, eliminated: false, contactTimer: 0 }));
  ...
  step(dt) {
    ...
    const accel = machine.accel * (boost ? 1.18 : 1) - (this.sim.speed > targetSpeed ? 20 : 0) - this.sim.speed * .018;
    ...
    if (Math.abs(this.sim.lateral) > edge) {
      ...
      this.sim.speed = Math.max(0, this.sim.speed - (6 + wallOver * 8) * dt); this.sim.energy = Math.max(0, this.sim.energy - (8 + wallOver * 12) * dt);
    ...
    if (Math.abs(this.sim.lateral) > magnetic) this.sim.lateral *= .985;
  ...
  updateAI(dt, data, track) {
    for (let i = 0; i < this.ai.length; i += 1) {
      ...
      const ideal = (this.selectedMachine().top / 3.6) * (.83 + rival.skill * .13 + rubber + Math.sin(this.simClock * (1.3 + rival.aggression) + i) * .012);
      rival.speed += (ideal - rival.speed) * (1 - Math.exp(-dt * (3.2 + rival.aggression)));
      rival.speed = clamp(rival.speed, 25, this.selectedMachine().top / 3.6 + 5);
      rival.distance += rival.speed * dt / Math.max(1, track.length); rival.progress = wrap01(.04 + rival.distance);
      const wanted = lineLateral(data, rival.progress) + Math.sin(this.simClock * (1.1 + rival.aggression) + i * 1.7) * (.12 + rival.aggression * .22);
  ...
  handleContacts(track) {
    this.sim.contactTimer -= FIXED;
    const playerFrame = this.frameAt(this.sim.progress, this.sim.lateral, this.contactFrame || (this.contactFrame = {}));
    for (let i = 0; i < this.ai.length; i += 1) {
      const rival = this.ai[i]; if (rival.eliminated) continue;
      if (Math.min(Math.abs(this.sim.progress - rival.progress), 1 - Math.abs(this.sim.progress - rival.progress)) > .012 || Math.abs(this.sim.lateral - rival.lateral) > 3.1) continue;
      ...
      if (this.sim.contactTimer <= 0) { this.sim.contactTimer = .24; this.sim.energy = Math.max(0, this.sim.energy - 7); this.sim.speed = Math.max(0, this.sim.speed - 2.5); this.sparkAtPlayer(0xff8b59, 8); sfx(kit, 'contact', { volume: .3 }); }
    }
  }
  ...
  finishRace() {
    ...
    const result = { position, time, track: track.id, mode: this.raceMode, points: POINTS[position - 1] || 0, medal: time < 72 ? 'GOLD' : time < 84 ? 'SILVER' : 'BRONZE' };
  ...
  updateView(viewDt) {
    ...
    const mainState = this.worldPacket.carState; mainState.position = playerFrame.position; mainState.progress = this.sim.progress; mainState.yaw = Math.atan2(playerFrame.tangent.x, playerFrame.tangent.z); mainState.speed = this.sim.speed; mainState.steering = this.sim.steering; mainState.acceleration = this.sim.acceleration; mainState.lateralG = this.sim.steering * this.sim.speed * .04; mainState.pitch = this.sim.airborne > 0 ? .18 : 0; mainState.roll = this.player ? this.player.roll : 0; mainState.boost = this.sim.boost;
    const rivalFrames = this.worldPacket.rivals;
    for (let i = 0; i < this.ai.length; i += 1) { const ai = this.ai[i]; if (!this.viewRivalFrames) this.viewRivalFrames = []; if (!this.viewRivalFrames[i]) this.viewRivalFrames[i] = blankFrame(); const frame = this.frameAt(ai.progress, ai.lateral, this.viewRivalFrames[i]); const packet = rivalFrames[i] || (rivalFrames[i] = {}); packet.position = frame.position; packet.progress = ai.progress; packet.speed = ai.speed; packet.steering = 0; packet.acceleration = 0; }
    rivalFrames.length = this.ai.length;
    this.racer.world.update(this.worldPacket, viewDt);
    if (this.player) this.player.update({ position: playerFrame.position, tangent: playerFrame.tangent, right: playerFrame.right, up: playerFrame.up, steering: this.sim.steering, acceleration: this.sim.acceleration, lateralG: this.sim.steering * this.sim.speed * .04, pitch: this.sim.airborne > 0 ? .18 : 0 }, viewDt, this.simClock, this.sim.energy, this.sim.boost);
    for (let i = 0; i < this.rivals.length; i += 1) { if (this.ai[i] && this.ai[i].eliminated) { this.rivals[i].root.visible = false; continue; } this.rivals[i].root.visible = true; const frame = this.frameAt(this.ai[i].progress, this.ai[i].lateral, this.viewRivalFrames[i]); this.rivals[i].update({ position: frame.position, tangent: frame.tangent, right: frame.right, up: frame.up, steering: 0, acceleration: 0 }, viewDt, this.simClock, 100, 0); }
    if (this.ghostCraft && this.ghost) { if (!this.ghostFrame) this.ghostFrame = blankFrame(); const ghostFrame = this.frameAt(this.ghost.progress, lineLateral(this.tracks[this.trackIndex] || fallbackTrack(this.trackIndex), this.ghost.progress), this.ghostFrame); this.ghostCraft.update({ position: ghostFrame.position, tangent: ghostFrame.tangent, right: ghostFrame.right, up: ghostFrame.up, steering: 0, acceleration: 0 }, viewDt, this.simClock, 100, 0); }
  ...
  renderRaceHud(ctx, w, h) {
    ...
    this.panel(ctx, w - 132, h - 78, 116, 42, 'rgba(4,22,35,.58)', 'rgba(91,230,238,.45)'); this.text(ctx, 'HOLD BOOST', w - 74, h - 57, 11, '#a9d8df', 'center', 700);
...
async function boot() {
  ...
  app = new IonwakeApp(tracks); kit.loader.hide(); requestAnimationFrame((time) => app.loop(time));
}
boot();
