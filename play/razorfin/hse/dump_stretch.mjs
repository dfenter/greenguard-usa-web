...
for (const file of process.argv.slice(2)) {
  ...
  for(let t=0;t+2<idx.length;t+=3){ const a=idx[t],b=idx[t+1],c=idx[t+2];
    for(const [p,q] of [[a,b],[b,c],[c,a]]){ const k=p<q?p+':'+q:q+':'+p; if(seen.has(k))continue; seen.add(k);
      const rl=el(R,p,q); if(rl<1e-9) continue; const ol=el(O,p,q); rows.push({s:ol/rl, rl, ol, p, q}); } }
  ...
  for (const r of rows.slice(0,8)) {
    console.log(`  stretch=${r.s.toFixed(3)} restL=${(r.rl/L).toExponential(2)}L openL=${(r.ol/L).toExponential(2)}L jawW=${w(mesh,jawIdx,r.p).toFixed(3)}/${w(mesh,jawIdx,r.q).toFixed(3)}`);
  }
