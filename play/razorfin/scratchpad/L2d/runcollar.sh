...
run(){ for i in $(seq 1 80); do
  ...
  if echo "$out" | grep -q "VERIFY base"; then echo "$out" | grep -E "VERIFY|GATE|COLLAR|PLATE max|missing" >> $O; return 0; fi
  sleep 20
done; echo "GAVEUP $1 $2" >> $O; }
