...
// axis fix first, then sweep exposure to land the median
await p.evaluate(()=>{ for(const m of window.__M){
  ...
  window.__ex=setInterval(()=>{ const r=(RF.ctx&&RF.ctx.renderer)||RF.Game.renderer;
    if(r&&window.__want!=null) r.toneMappingExposure=window.__want; },16); });
for(const x of [0.80,0.76,0.72,0.68,0.64]){
  await p.evaluate(v=>{window.__want=v;},x);
  await shot(`expo${x}`); }
