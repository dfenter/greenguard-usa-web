...
(function () {
  ...
  function buildSheet(scene, key, families, draw) {
    ...
    var baked = GGKit.hiDpi.canvas(cell * count, cell), canvas = baked.canvas, c = baked.ctx, index = 0, fi, si, fr;
    ...
    for (fi = 0; fi < families.length; fi++) for (si = 0; si < states.length; si++) for (fr = 0; fr < 2; fr++) { texture.add(families[fi] + '-' + states[si] + '-' + fr, 0, Math.round(index * cell * RETINA_FACTOR), 0, Math.round(cell * RETINA_FACTOR), Math.round(cell * RETINA_FACTOR)); index++; }
  }
  ...
  Scene.prototype.buildTextures = function () {
    var baked = GGKit.hiDpi.canvas(W, H), bg = baked.canvas, c = baked.ctx, x, y;
  ...
  Scene.prototype.buildWorld = function () {
    this.boardImage = this.add.image(W / 2, H / 2, 'gm-board').setDisplaySize(W, H).setDepth(0); this.bandTint = this.add.rectangle(W / 2, 438, 350, 558, 0x0b1822, 0.12).setDepth(1);
    this.roomTiles = []; this.roomProps = []; this.lightViews = []; this.artifactViews = [];
    var row, col, tile, image, p;
    for (row = 0; row < 16; row++) for (col = 0; col < 11; col++) { p = tileCenter(col, row); image = this.add.image(p.x, p.y, 'gm-wall').setDisplaySize(TILE, TILE).setDepth(2); image.baseX = p.x; image.baseY = p.y; this.roomTiles.push(image); }
    for (row = 0; row < 16; row++) for (col = 0; col < 11; col++) { p = tileCenter(col, row); image = this.add.image(p.x, p.y, 'gm-light').setDisplaySize(89.6, 89.6).setDepth(3).setAlpha(0); image.baseX = p.x; image.baseY = p.y; this.lightViews.push(image); }
    this.torchProps = []; this.chamberProps = []; this.altarProp = this.add.image(0, 0, 'gm-altar').setDisplaySize(32, 32).setDepth(5).setVisible(false); this.entranceProp = this.add.image(0, 0, 'gm-entrance').setDisplaySize(32, 32).setDepth(5).setVisible(false);
    for (row = 0; row < 16; row++) for (col = 0; col < 11; col++) { p = tileCenter(col, row); image = this.add.image(p.x, p.y, 'gm-torch').setDisplaySize(32, 32).setDepth(5).setVisible(false); image.baseX = p.x; image.baseY = p.y; this.torchProps.push(image); image = this.add.image(p.x, p.y, 'gm-chamber').setDisplaySize(32, 32).setDepth(5).setVisible(false); image.baseX = p.x; image.baseY = p.y; this.chamberProps.push(image); }
    for (row = 0; row < 2; row++) { image = this.add.image(0, 0, 'gm-artifact').setDisplaySize(32, 32).setDepth(8).setVisible(false); this.artifactViews.push(image); }
    this.worldObjects = this.roomTiles.concat(this.torchProps, this.chamberProps, [this.altarProp, this.entranceProp]);
    this.arenaPulse = this.add.image(195, 646, 'gm-ring').setDisplaySize(64, 64).setDepth(4).setAlpha(0); this.doorImage = this.add.image(328, 384, 'gm-entrance').setDisplaySize(23.04, 23.04).setDepth(5);
    ...
    this.heroShadow = this.add.image(195, 646, 'gm-shadow').setDisplaySize(67.5, 67.5).setDepth(6).setAlpha(0.72); this.heroLight = this.add.image(195, 646, 'gm-light').setDisplaySize(121.6, 121.6).setDepth(7).setAlpha(0.5); this.heroView = this.add.sprite(195, 646, 'gm-hero', 'hero-idle-0').setDepth(10).setDisplaySize(62, 62);
    this.enemyViews = []; this.hazardViews = []; this.fxViews = []; this.numberViews = [];
    for (row = 0; row < MAX_ENEMIES; row++) this.enemyViews.push({ sprite: this.add.sprite(0, 0, 'gm-enemy', 'shade-idle-0').setDepth(8).setDisplaySize(62, 62).setVisible(false), bar: this.add.rectangle(0, 0, 50, 5, PAL.teal).setOrigin(0.5).setDepth(9).setVisible(false), intent: this.add.image(0, 0, 'gm-ring').setDisplaySize(38.4, 38.4).setDepth(7).setAlpha(0).setVisible(false) });
    for (row = 0; row < MAX_HAZARDS; row++) this.hazardViews.push(this.add.image(0, 0, 'gm-ring').setDisplaySize(51.2, 51.2).setDepth(4).setVisible(false));
  ...
  Scene.prototype.usePulse = function () { if (!this.run || this.run.phase !== 'playing' || kit.paused || this.hero.pulse > 0) return; this.hero.pulse = 7; this.run.tutorial.pulse = true; this.pulseStartedAt = this.simTime; this.arenaPulse.setPosition(this.hero.x, this.hero.y).setDisplaySize(51.2, 51.2).setAlpha(.8); var dps = currentDps(profile.gear), hits = 0, i; for (i = 0; i < MAX_ENEMIES; i++) if (this.enemies[i].alive && dist(this.hero.x, this.hero.y, this.enemies[i].x, this.enemies[i].y) <= 142) { this.damageEnemy(this.enemies[i], dps * 2.65 * relicMultiplier('pulse'), PAL.cyan, .35); hits++; } this.emitFx(this.hero.x, this.hero.y, PAL.cyan, 34, 175, 'spark'); kit.audio.sfx('pulse', { volume: .7 }); if (!hits) this.showTransient('PULSE MISSED / KEEP MOVING', PAL.dim, .75); this.updateTutorial(); };
  ...
  Scene.prototype.renderViews = function (juice) {
    ...
    var pulseAge = this.simTime - this.pulseStartedAt, pulseScale = pulseAge >= 0 && pulseAge < .72 ? .4 + pulseAge * 1.2 : .4; this.arenaPulse.setAlpha(pulseAge >= 0 && pulseAge < .72 ? clamp(.8 - pulseAge * 1.2, 0, .8) : 0).setDisplaySize(128 * pulseScale, 128 * pulseScale);
  ...
  var config = { type: Phaser.CANVAS, parent: 'game', backgroundColor: '#080b14', render: { pixelArt: true, antialias: false, roundPixels: true, clearBeforeRender: true }, scale: { mode: Phaser.Scale.FIT, autoCenter: Phaser.Scale.CENTER_BOTH, width: W, height: H }, fps: { target: 60, min: 30 }, scene: Scene };
