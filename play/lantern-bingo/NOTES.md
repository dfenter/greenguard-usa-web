...
`this.cameras.main.setScroll(-frame.dx, -frame.dy);` in the play scene update.
The zoom is applied through a `configureRetinaScene(scene)` helper rather than inline.
...
`node release_gate.mjs http://localhost:<port> 1 lantern-bingo`, headless Chrome,
...
| setZoom + setOrigin(0,0) | 25904 | 22.4% | 3x | READY (all checks pass) |
