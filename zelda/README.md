...
- **Eight dungeons**: Levels 1–8, each a multi-room layout with locked
  ...
  is gated behind the **Raft** (found in an overworld cave). Collect all eight
  pieces to open the final mountain path.
- **Finale**: Level 9 **DEATH'S CROWN**, Patra, the invisible Ganon fight with
  sword stun reveals and a silver-arrow kill, the Red Ring, Zelda rescue, and
  the ending screen.
- **Audio**: a pattern-based multi-track music engine for title, overworld,
  dungeons, Level 9, bosses, Ganon, and the ending, plus pickup, cave, and
  low-health cues. Everything is synthesized with WebAudio.
- **Item-hoist pose**: major gear and B-item pickups briefly lock Link while
  he raises the new item overhead.
- **HUD polish**: persistent mute speaker icon, potion charge pips, minimap,
  inventory slots, and the Level 9 DEATH'S CROWN banner.
- **Offline PWA**: a versioned precache service worker caches the HTML,
  manifest, and every JavaScript game module for offline play.
- **Title / Game Over / Win** screens, ending sequence, sound effects, and
  procedural music.
...
| `sound.js` | WebAudio SFX + pattern music engine |
