...
export default function GtmCompetitivePage({ product = DEFAULT_PRODUCT, session }) {
  const COLUMNS = [
    ['competitor', 'Competitor / approach'],
    ['best_at', 'Best at'],
    ['difference', product === 'sparkbridge' ? 'Where SparkBridge differs' : 'Where we differ'],
  ...
  async function load() {
    setLoading(true)
    const data = await fetch(`/api/gtm/library?kind=competitive&product=${product}`).then((r) => r.json())
    setRows(data.rows || [])
    setLoading(false)
  }
  ...
  async function submit(e) {
    ...
    const res = await fetch('/api/gtm/library', {
      ...
      body: JSON.stringify({ kind: 'competitive', data: form, product }),
    })
  ...
  return (
    <GtmLayout title="Competitive" session={session} product={product}>
      <Head><title>GTM Competitive · GreenGuard USA</title></Head>
      <p>Living document. Honest by rule: name what each does well. Where {product === 'sparkbridge' ? 'a module-MQTT cell reads the same as ours' : 'an approach reads the same as ours'}, say so.</p>
