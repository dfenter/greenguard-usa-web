...
export default function GtmObjectionsPage({ product = DEFAULT_PRODUCT, session }) {
  ...
  async function load() {
    setLoading(true)
    const data = await fetch(`/api/gtm/library?kind=objection&product=${product}`).then((r) => r.json())
    setRows(data.rows || [])
    setLoading(false)
  }
  ...
  async function submit(e) {
    ...
    const res = await fetch('/api/gtm/library', {
      ...
      body: JSON.stringify({ kind: 'objection', data: form, product }),
    })
  ...
  return (
    <GtmLayout title="Objections" session={session} product={product}>
