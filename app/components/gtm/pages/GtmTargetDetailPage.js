...
export default function GtmTargetDetailPage({ product = DEFAULT_PRODUCT, session, row }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      const [touchRes, reportRes, dealRes] = await Promise.all([
        fetch(`/api/gtm/touches?firm=${encodeURIComponent(row.firm)}&product=${product}`).then((r) => r.json()),
        fetch(`/api/gtm/call-reports?firm=${encodeURIComponent(row.firm)}&product=${product}`).then((r) => r.json()),
        fetch(`/api/gtm/deals?firm=${encodeURIComponent(row.firm)}&product=${product}`).then((r) => r.json()),
      ])
      if (cancelled) return
  ...
  async function saveStage(newStage) {
    ...
    await fetch('/api/gtm/deals', {
      ...
      body: JSON.stringify({ firm: row.firm, stage: newStage, product }),
    })
  }
  ...
  return (
    <GtmLayout title={row.firm} session={session} product={product}>
