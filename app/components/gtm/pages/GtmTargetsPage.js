...
import { DEFAULT_PRODUCT, scoreFieldsFor, tierFor } from '../../../lib/gtm-products'

export default function GtmTargetsPage({ product = DEFAULT_PRODUCT, session }) {
  ...
  const scoreFields = scoreFieldsFor(product)

  async function syncFromHubSpot() {
    setSyncing(true)
    setSyncResult(null)
    try {
      const res = await fetch(`/api/gtm/hubspot-sync?product=${product}`, { method: 'POST' })
  ...
  async function load() {
    setLoading(true)
    const data = await fetch(`/api/gtm/targets?product=${product}`).then((r) => r.json())
    setRows(data.rows || [])
    setLoading(false)
  }
  ...
  return (
    <GtmLayout title="Targets" session={session} product={product}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 16, alignItems: 'center' }}>
        ...
        <button
          type="button"
          onClick={() => { window.location.href = `/api/gtm/targets/export.csv?product=${product}` }}
...
function TargetDrawer({ row, session, product, scoreFields, onClose, onSaved }) {
  const [scores, setScores] = useState(() => {
    const s = row.myScore || {}
    return scoreFields.reduce((acc, [k]) => ({ ...acc, [k]: s[k] ?? 0 }), {})
  })
  ...
  const total = scoreFields.reduce((sum, [k]) => sum + Number(scores[k] || 0), 0)
  const tier = tierFor(product, total)
  ...
  async function saveScore() {
    ...
    const res = await fetch('/api/gtm/targets/score', {
      ...
      body: JSON.stringify({ firm: row.firm, product, ...scores }),
    })
    const data = await res.json()
    if (!res.ok) {
      setSaving(false)
  ...
  async function saveVertical() {
    await fetch('/api/gtm/targets/vertical', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ firm: row.firm, vertical, product }),
    })
    onSaved()
  }
  ...
  async function saveApproval() {
    const res = await fetch('/api/gtm/targets/approve', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ firm: row.firm, approved: !approved, dan_notes: danNotes, product }),
    })
    const data = await res.json()
    if (res.ok) {
  ...
  async function logTouch() {
    ...
    await fetch('/api/gtm/touches', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ firm: row.firm, person, touch: touchNum, channel, product }),
    })
    onSaved()
  }
