...
export default function GtmTimelinePage({ product = DEFAULT_PRODUCT, session, items = [] }) {
  ...
  async function toggleItem(itemId) {
    ...
    try {
      const res = await fetch('/api/gtm/progress', {
        ...
        body: JSON.stringify({ item_id: itemId, done: !prev, product }),
      })
  ...
  async function saveStartDate() {
    ...
    try {
      const res = await fetch('/api/gtm/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ key: 'start_date', value: startInput, product }),
      })
  ...
  return (
    <GtmLayout title="Timeline" session={session} progressPct={progressPct} product={product}>
