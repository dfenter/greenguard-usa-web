...
export const getServerSideProps = gtmServerSideProps(() => {
  const leadershipPage = getPage('11-reports/leadership-review-template')
  ...
  useEffect(() => {
    ...
    async function load() {
      const [kpisRes, historyRes] = await Promise.all([
        fetch('/api/gtm/kpis').then((r) => r.json()),
        fetch('/api/gtm/weekly-reports').then((r) => (r.ok ? r.json() : { rows: [] })).catch(() => ({ rows: [] })),
      ])
      if (cancelled) return
      setCounts({
  ...
  async function save() {
    ...
    try {
      const res = await fetch('/api/gtm/weekly-reports', {
        ...
        body: JSON.stringify({ week_start: weekStart, counts, notes }),
      })
