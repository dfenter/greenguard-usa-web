...
function StatusDot({ ok, warning }) {
  const color = warning ? 'var(--warn)' : ok ? 'var(--ok)' : 'var(--danger)'
...
function ServiceRow({ name, result }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: '1px solid rgba(var(--green-rgb),0.08)' }}>
      <StatusDot ok={result.ok} warning={hasWarning} />
      <div style={{ flex: 1 }}>
        <div style={{ fontWeight: 700, fontSize: '0.88rem' }}>{label}</div>
        {result.error && <div style={{ fontSize: '0.75rem', color: 'var(--danger)', marginTop: 2 }}>{result.error}</div>}
        {result.errors?.length > 0 && result.errors.map((e, i) => (
          <div key={i} style={{ fontSize: '0.75rem', color: 'var(--danger)', marginTop: 2 }}>{e}</div>
        ))}
        {hasWarning && <div style={{ fontSize: '0.75rem', color: 'var(--warn)', marginTop: 2 }}>{result.warning}</div>}
      </div>
      {result.latency != null && (
      ...
      <span style={{ fontSize: '0.72rem', fontWeight: 700, color: result.ok ? 'var(--ok)' : 'var(--danger)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, padding: '16px 20px', borderRadius: 10, background: 'var(--bg-alt)', border: '1px solid var(--border)', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            <button
              onClick={runCheck}
              disabled={loading}
              style={{ padding: '8px 18px', borderRadius: 6, border: 'none', cursor: loading ? 'not-allowed' : 'pointer', fontWeight: 800, fontSize: '0.82rem', background: loading ? 'rgba(var(--gold-rgb),0.20)' : 'var(--gold)', color: loading ? 'var(--text-dim)' : 'var(--text-on-accent)' }}
            >
