...
export default function InvoicesList() {
  ...
  const chip = (active, color) => ({
    ...
    fontSize: '0.78rem', fontWeight: 800, cursor: 'pointer', fontFamily: 'Nunito Sans, sans-serif',
  })
        ...
        <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid rgba(122,171,130,0.15)', marginBottom: 14, overflowX: 'auto' }}>
          {TABS.map((t) => (
            <button key={t.key} onClick={() => setTab(t.key)}
              style={{ padding: '10px 16px', background: 'transparent', border: 'none', borderBottom: tab === t.key ? '2px solid #7dffaa' : '2px solid transparent', color: tab === t.key ? '#7dffaa' : 'rgba(212,230,202,0.55)', fontWeight: 800, fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'Nunito Sans, sans-serif', whiteSpace: 'nowrap' }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {(loading || hasMore) && (
          <div style={{ marginTop: 12, textAlign: 'center' }}>
            {loading
              ...
              : <button onClick={() => loadPage(cursor)} style={{ padding: '7px 14px', borderRadius: 5, border: '1px solid rgba(122,171,130,0.25)', background: 'transparent', color: 'rgba(212,230,202,0.7)', cursor: 'pointer', fontWeight: 700, fontSize: '0.78rem', fontFamily: 'Nunito Sans, sans-serif' }}>
                  Load 100 more
                </button>
            }
          </div>
