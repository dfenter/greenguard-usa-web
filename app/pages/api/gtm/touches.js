...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const { rows } = await q(
      `SELECT id, firm, person, touch, channel, sent_at, reply, notes FROM gtm_touches WHERE firm = $1 ORDER BY sent_at DESC`,
      [firm]
    )
  ...
  if (req.method === 'POST') {
    ...
    const result = await q(
      `INSERT INTO gtm_touches (firm, person, touch, channel, reply, notes)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
      [firm, person, touch, channel, Boolean(reply), typeof notes === 'string' ? notes : null]
    )
