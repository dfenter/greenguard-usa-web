...
export default async function handler(req, res) {
  ...
  await q(
    `INSERT INTO gtm_targets_state (firm, approved, approved_by, approved_at, dan_notes, updated_at)
     VALUES ($1, $2, $3, now(), $4, now())
     ON CONFLICT (firm) DO UPDATE SET approved = $2, approved_by = $3, approved_at = now(), dan_notes = $4, updated_at = now()`,
    [firm, approved, session.email, typeof dan_notes === 'string' ? dan_notes : null]
  )

  const sheet = await writeTargetCells(firm, { approve: approved ? 'Y' : 'N', dan_notes: dan_notes || '' })
