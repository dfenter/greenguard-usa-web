...
const { isValidSubScore } = require('../../../../lib/gtm-score')
const { writeTargetCells } = require('../../../../lib/gtm-sheets')
const { resolveProduct, tierFor } = require('../../../../lib/gtm-products')
...
function scoreTotalAndTierFor(product, scores) {
  ...
  return { total, tier: tierFor(product, total) }
}
...
export default async function handler(req, res) {
  ...
  let total, tier
  try {
    ;({ total, tier } = scoreTotalAndTierFor(product, scores))
  } catch (e) {
  ...
  const sheet = await writeTargetCells(firm, cellValues, product)
