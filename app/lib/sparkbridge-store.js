...
// customers, so a gateway name alone never matches.
...
function ensureSchema() {
  if (!_schema) {
    _schema = (async () => {
      await q(`CREATE TABLE IF NOT EXISTS sparkbridge_grants (
      ...
      await q(`CREATE INDEX IF NOT EXISTS sparkbridge_grants_pair ON sparkbridge_grants (email, gateway_id)`)
      await q(`CREATE TABLE IF NOT EXISTS sparkbridge_keys (
        id serial PRIMARY KEY,
        email text NOT NULL,
        gateway_id text NOT NULL,
        licensee text,
      ...
      await q(`CREATE INDEX IF NOT EXISTS sparkbridge_keys_pair ON sparkbridge_keys (email, gateway_id)`)
...
async function activeGrants(email, gateway) {
  ...
  const r = await q(
    `SELECT id, email, gateway_id, gateway_display, licensee, sku, entitlements, support_until, session_id, status, created_at
      ...
      WHERE lower(email) = $1 AND lower(gateway_id) = $2 AND status = 'active'
  ...
  try {
    ...
    await client.query(
      `UPDATE sparkbridge_keys SET status = 'superseded', superseded_at = now(), superseded_by = $3
        WHERE lower(email) = $1 AND lower(gateway_id) = $2 AND status = 'active' AND id <> $3`,
...
module.exports = { ensureSchema, recordGrant, activeGrants, recordIssuedKey, normEmail, normGateway, displayGateway }
