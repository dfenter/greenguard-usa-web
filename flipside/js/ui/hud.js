...
// .hud-pips > .pip / .pip.full, .world-tag, .toast, .banner,
...
export function createHud(G, hooks = {}) {
  ...
  function updateScreens(state, status) {
    ...
    if (touchControls) {
      const showControls = status === 'playing' || status === 'flipping' || status === 'clearing';
  ...
  function updatePips(state, status) {
    ...
    if (chargeChanged || affordable !== lastFlipAffordable) {
      ...
      flipButton.title = affordable ? 'Flip the page' : 'No flip charges available';
